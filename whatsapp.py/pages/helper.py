# all of this is for the group chat
...
def preprocess_chat_data(file):
    ...
    selected_users = st.sidebar.multiselect("Selected users", df['User']) # Changed column name to 'user_name'
    ...
    all_words = ' '.join([word for word in df['Message'] if isinstance(word, str)])
    word_count = len(re.findall(r'\w+', all_words))
    st.write("Total Number of Words:", word_count)
